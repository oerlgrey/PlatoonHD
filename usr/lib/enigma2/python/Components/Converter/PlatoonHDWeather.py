		...
		if WEATHER_LOAD == True:
			try:
				...
				if r != None and r <= 1.5:
					...
					res = requests.get('http://weather.service.msn.com/data.aspx?src=windows&weadegreetype=C&culture=' + str(config.plugins.PlatoonHD.msn_language.value) + '&wealocations=wc:' + str(config.plugins.PlatoonHD.msn_code.value), timeout=1.5)
